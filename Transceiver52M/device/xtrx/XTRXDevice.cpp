...
// This is an empirically selected value which depends on samplerates,
// filter configuration and other frontend parameters. But we support only
// one samplerate right now (4 SPS for both Rx and Tx) and other configuration
// is fixed so we need just one value for now.
...
int XTRXDevice::open(const std::string &args, int ref, bool swap_channels)
{
	...
	if (res) {
		LOG(ALERT) << "Failed to open an XTRX device \"" << xtrx_name << "\", code " << res;
		return -1;
	}
...
double XTRXDevice::setTxGain(double dB, size_t chan)
{
	...
	int res = xtrx_set_gain(device, XTRX_CH_AB, XTRX_TX_PAD_GAIN, dB, &txGain);
	if (res) {
